...
function Customer() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            var res = await axios.post(`${api_url}create`, { ...userData, tanggal_masuk: tanggalMasuk, tanggal_keluar: tanggalKeluar });
        ...
        return custArr.map((el, index) => {
            return (
                <div key={el.id} className='grid grid-cols-8 gap-3 text-left text-xs border-b py-3 items-center'>
                    ...
                    <div>{el.tanggal_keluar != "" ? new Date(el.tanggal_keluar).toLocaleDateString() : "-"}</div>
